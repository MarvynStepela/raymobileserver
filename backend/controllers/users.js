...
export const lockUser = async (req, res) => {
    ...
    db.query(
      "SELECT * FROM users WHERE userID=? AND is_verified = ? limit 1",
      [req.params.userid, 1],
      function (error, result, fields) {
        ...
        if (result.length > 0) {
          db.query(
            `UPDATE users SET is_verified = 2 WHERE userID = '${req.params.userid}'`,
            (err, resultat) => {
              if (err) throw err;

              if (resultat) {
                return res.status(200).send({
                  msg: "Compte vérouillé avec succès",
                  code: 100,
                });
              }
            }
...
export const unLockockUser = async (req, res) => {
  db.query(
    "SELECT * FROM users WHERE userID=? AND is_verified = ? limit 1",
    [req.params.userid, 2],
    function (error, result, fields) {
      ...
      if (result.length > 0) {
        ...
        db.query(
          `UPDATE users SET is_verified = 1 WHERE userID = '${req.params.userid}'`,
          (err, resultat) => {
            if (err) throw err;

            if (resultat) {
              return res.status(200).send({
                msg: "Compte déverouillé avec succès",
                code: 100,
              });
            }
          }
